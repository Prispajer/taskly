...
WORKDIR /src
COPY ["Taskly.API/Taskly.API.csproj", "Taskly.API/"]
RUN dotnet restore "./Taskly.API/Taskly.API.csproj"
COPY . .
WORKDIR "/src/Taskly.API"
RUN dotnet build "./Taskly.API.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Ten etap służy do publikowania projektu usługi do skopiowania do etapu końcowego
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./Taskly.API.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
